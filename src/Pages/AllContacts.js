...
const AllContact = () => {
  ...
  useEffect(async () => {
    setLoading(true);
    try {
      const res = await fetch(`https://contact-management-api-seven.vercel.app/api/mycontacts`, {
        method: "GET",
        headers: {
  ...
  const deleteContact = async (id) => {
    if (window.confirm("are you sure you want to delete this contact ?")) {
      try {
        const res = await fetch(`https://contact-management-api-seven.vercel.app/api/delete/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
